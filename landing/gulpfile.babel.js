...
gulp.task('b-css', () => {
    return gulp
            ...
            .pipe(sass({
                ...
                style: 'nested',
                onError: () => {
                    console.log('Ooops!! Something happens when compile SASS')
                }
            }))
...
gulp.task('watch', () => {
    gulp.watch('src/*.html', gulp.series('b-html'));
    gulp.watch('src/styles', gulp.series('b-css'));
    gulp.watch(PATH.jsSrc.concat('**/*.js'), gulp.series('b-js'));
    gulp.watch(PATH.imgSrc.concat('**/*.+(png|jpg|jpeg|gif|svg)'), gulp.series('b-img'));
});

gulp.task('build', gulp.series('b-html', 'b-css', 'b-js', 'b-img'), () => {
    return igniteServer();
});
